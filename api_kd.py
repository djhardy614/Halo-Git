...
player_roster = ['BudbudHardy', 'Flaresman', 'Dead1n5ide', 'RustlingSpore',
                 'Sashwank', 'ManChivster', 'UBERmatto', 'Fro5tShark', 'r3dFLash']
...
def data_collect(gamertag):
    ...
    try:
        ...
        conn.request(
            "GET", f"/stats/h5/players/{gamertag}/matches?modes=custom", "{body}", headers)
    ...
    player_kda = round((sum(player_kills_count) + (1 / 3 *
                                                   (sum(player_assist_count)))) / sum(player_death_count), 3)
